...
import { AlertTriangle, CalendarDays, Copy, Eye, EyeOff, FlaskConical, GitBranch, Layers, Plus, RotateCcw, Save, Sliders, Sparkles, Wand2, Zap } from 'lucide-react'
import { toast } from 'sonner'
import { useAcademic, useStore } from '@/lib/store'
import { ApiError, api, errorMessage } from '@/lib/api'
import type { AutoGenerateDraftEntry, AutoGenerateMode, AutoGenerateResult, Diagnostic, PeriodDef, RefineDraftEntry, RefineDraftResult, VersionEntry, WhatIfResult } from '@/lib/data'
import {
  DAY_LABELS, cellKey, daysFor, findBlockingLock, sortedPeriods, useAutoGenerate, useEditEvents, useEntryLookup, useFetch, useGenerationJob, useGridsFor,
  useOverrideActions, useOverrideSession, useRefineDraft, useUndo, useVersionLocks, useWhatIf,
...
import { DiagnosticsPanel, FreeCell, PeriodCard, TimetableGrid } from './timetable'
import { CellActionsModal, HistoryPanel, LockBadge, LocksPanel, PublishedImmutableBanner, VersionStatusBar } from './timetableOverrides'
import { WhatIfReviewModal, WhatIfSetupModal, WhatIfVersionBanner } from './timetableWhatIf'
...
/** Every possible school weekday (Mon–Sat) — the override system's Move/Swap target picker and the Locks
 * panel's DAY-lock picker both need to reach a day the grid isn't currently showing any period on (e.g.
 * moving something onto an as-yet-unused Saturday), unlike the grid's own display days (`sessionDays`/
 * `days`, which only show a day once something is actually scheduled on it). */
const ALL_WEEKDAYS = [1, 2, 3, 4, 5, 6]

export function TimetableBuilderMod() {
  const { db } = useStore()
  const { classes, terms, currentYear, currentTerm, classSubjects, subjectById, rooms, templateFor, periodTemplates, classById } = useAcademic()
  ...
  // Phase T7 — versioning + locks + the management-override system (roadmap D10, this phase's centerpiece).
  // See .agents/edunova/phase-t7-versioning-override.md and server/src/modules/timetable/{versions,locks,
  // overrides}.ts. `overrideSession` resolves whether this class/term is currently governed by a PUBLISHED
  // version (immutable — the plain content-edit path below is refused server-side, 409) and whether an
  // editable draft already exists for it (auto-resumed). While `inSession` is true, cell interaction below
  // hands off to CellActionsModal (move/swap/regenerate-slot/lock, all with live dryRun conflict preview)
  // instead of the plain subject/room/teacher editor — the override system has no "author new content"
  // primitive by design (phase-t7 §4 lists move/swap/lock/unlock/regenerate-slot/undo, not content authoring).
  const overrideSession = useOverrideSession(classId, termId)
  const activeVersion = overrideSession.activeVersion
  const inSession = !!activeVersion
  const editLocked = !!overrideSession.governingPublished && !inSession
  const locksHook = useVersionLocks(activeVersion?.id)
  const editEvents = useEditEvents(activeVersion?.id)
  const reloadSession = useCallback(() => { overrideSession.reload(); locksHook.reload(); editEvents.reload() }, [overrideSession, locksHook, editEvents])
  const overrideActions = useOverrideActions(activeVersion?.id, reloadSession)
  const undoHook = useUndo(activeVersion?.id, reloadSession)
  const [showLocks, setShowLocks] = useState(false)
  const [showHistory, setShowHistory] = useState(false)
  const [sessionCell, setSessionCell] = useState<{ entry: VersionEntry; period: PeriodDef } | null>(null)
  const sessionEntries = useMemo(() => (activeVersion?.entries ?? []).filter(e => e.classId === classId), [activeVersion, classId])
  const sessionByKey = useMemo(() => new Map(sessionEntries.map(e => [cellKey(e.dayOfWeek, e.periodIdx), e])), [sessionEntries])
  const sessionDays = daysFor(sessionEntries, showSat)
  const teacherOptions = useMemo(() => db.users.filter(u => u.role === 'teacher').map(u => ({ id: u.id, name: u.name })).sort((a, b) => a.name.localeCompare(b.name)), [db.users])
  const handleUndo = async () => {
    const res = await undoHook.undo()
    if (res) toast.success(`Undid change #${res.revertedEventSeq}`)
  }
  const handleRevertTo = async (seq: number) => {
    const res = await undoHook.revertTo(seq)
    if (res) toast.success(`Reverted ${res.revertedEventSeqs.length} change${res.revertedEventSeqs.length === 1 ? '' : 's'}`)
  }
  ...
  const save = async () => {
    ...
    try {
      ...
      const body = e instanceof ApiError && e.status === 409 ? (e.body as { conflicts?: Conflict[]; publishedVersionId?: string } | undefined) : undefined
      if (body?.publishedVersionId) {
        overrideSession.reload()
        toast.error('This timetable is published — start an edit session to make changes')
  ...
  // Auto-generate: a setup step (cohort scope/mode/confirm) followed by a draft review that stands apart
  ...
  const [autoDraft, setAutoDraft] = useState<{ result: AutoGenerateResult; mode: AutoGenerateMode; termId: string; cohortIds: string[]; diagnostics?: Diagnostic[] } | null>(null)
  const [commitErr, setCommitErr] = useState<string | null>(null)
  // Phase T5 §2 — SA refinement, an additional step between generate and commit; decomposes cleanly by
  // grade using `cohortIds` (see phase-t5-full-solver.md §2). `refineStats` folds into the SAME summary
  // strip AutoGenerateReview already renders rather than a second bolted-on panel, per D9.
  const refineSvc = useRefineDraft()
  const [refineStats, setRefineStats] = useState<RefineDraftResult | null>(null)
  // Phase T6 §3/§4 — when a generation run leaves anything unplaced, a real TimetableGenerationJob run
  // (same scope/mode) is fired to get the deterministic structured diagnosis (§4) in place of a bare "needs
  // manual attention" list — see AutoGenerateReview's DiagnosticsPanel below. The job runs synchronously
  // today (jobs.ts), so `diagJob.busy` is the only "progress" visible for now, but it's wired against the
  // real polling hook so it keeps working unchanged once generation is genuinely async.
  const diagJob = useGenerationJob()

  const cohorts = useEntity('cohorts')
  const genYearId = cls?.academicYearId ?? currentYear?.id
  const yearCohorts = useMemo(
    () => cohorts.items.filter(c => c.academicYearId === genYearId).sort((a, b) => Number(b.autoGenerated) - Number(a.autoGenerated) || a.name.localeCompare(b.name)),
    [cohorts.items, genYearId],
  )
  const [selectedCohortIds, setSelectedCohortIds] = useState<string[]>([])
  const toggleCohort = (id: string) => setSelectedCohortIds(ids => ids.includes(id) ? ids.filter(x => x !== id) : [...ids, id])
  // Defaults the picker to the currently-open class's own auto-generated section cohort — same one-click
  // convenience "This class" used to offer, without keeping the legacy scope option itself around.
  const openAutoSetup = () => {
    setAutoMode('fill-empty'); setAutoConfirm(false); setAutoErr(null)
    const ownCohort = cohorts.items.find(c => c.autoGenerated && c.classIds.includes(classId))
    setSelectedCohortIds(ownCohort ? [ownCohort.id] : [])
    setAutoSetupOpen(true)
  }

  // Phase T8 — what-if / partial re-optimization (roadmap T8, phase-t8-what-if.md). Patches whatever
  // version is most relevant right now — the draft being hand-edited if a session is active, else the
  // published version, else the most recent version of any status — never requires starting an edit session
  // just to ask "what would this change ripple into". A successful run creates a real new GENERATED version
  // (parented to whichever one was patched) that `overrideSession.reload()` then auto-resumes into exactly
  // like any other draft, so review/approve/publish is 100% the existing T7 flow — no parallel mechanism.
  const patchTargetVersion = activeVersion ?? overrideSession.governingPublished
    ?? [...overrideSession.versions].sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0]
  const whatIf = useWhatIf()
  const [whatIfSetupOpen, setWhatIfSetupOpen] = useState(false)
  const [whatIfResult, setWhatIfResult] = useState<WhatIfResult | null>(null)
  const cohortsByClassId = useMemo(() => {
    const m = new Map<string, { id: string; name: string }[]>()
    for (const c of cohorts.items) for (const cid of c.classIds) m.set(cid, [...(m.get(cid) ?? []), { id: c.id, name: c.name }])
    return m
  }, [cohorts.items])
  const runWhatIf = async (changeEvent: Parameters<typeof whatIf.run>[0]['changeEvent']) => {
    if (!patchTargetVersion) return
    const out = await whatIf.run({ versionId: patchTargetVersion.id, changeEvent })
    if (out) { setWhatIfSetupOpen(false); setWhatIfResult(out) }
  }
  const openWhatIfInGrid = () => { setWhatIfResult(null); overrideSession.reload() }

  const runAutoGenerate = async () => {
    setAutoErr(null)
    diagJob.reset()
    try {
      const result = await autoGen.generate({ termId, mode: autoMode, cohortIds: selectedCohortIds })
      setAutoDraft({ result, mode: autoMode, termId, cohortIds: selectedCohortIds })
      setRefineStats(null)
      setAutoSetupOpen(false)
      // Phase T6 §4 — anything left unplaced gets a real diagnosis: fire a TimetableGenerationJob at the
      // same scope/mode (deterministic — same DB state, same solver) purely to get its computed
      // `diagnostics[]`, which the draft-review screen renders instead of a bare reason list.
      if (result.unplaced.length > 0 && selectedCohortIds.length > 0) {
        const job = await diagJob.run({ termId, cohortIds: selectedCohortIds, mode: autoMode })
        if (job?.diagnostics?.length) setAutoDraft(d => d ? { ...d, diagnostics: job.diagnostics } : d)
      }
    } catch (e) { setAutoErr(errorMessage(e)) }
  }
  const discardDraft = () => { setAutoDraft(null); setCommitErr(null); setRefineStats(null); diagJob.reset() }
  ...
  // Decorates refine's response (which only carries the fields it actually needs — no classLabel/
  // subjectColor/roomName/teacherName) back into full AutoGenerateDraftEntry rows for display, using the
  // pre-refine draft as the source of truth for anything refine's moves can't change (subject/class
  // identity) and live lookups for anything a move CAN change (room, on a lab-block relocate).
  const roomById = useMemo(() => new Map(rooms.map(r => [r.id, r])), [rooms])
  const decorateRefined = useCallback((refined: RefineDraftEntry[], original: AutoGenerateDraftEntry[]): AutoGenerateDraftEntry[] => {
    const byClassSubject = new Map(original.map(d => [d.classSubjectId, d]))
    return refined.map(e => {
      const orig = byClassSubject.get(e.classSubjectId)
      return {
        classId: e.classId, classLabel: orig?.classLabel ?? classById.get(e.classId)?.label ?? e.classId,
        dayOfWeek: e.dayOfWeek, periodIdx: e.periodIdx, classSubjectId: e.classSubjectId,
        subjectName: e.subjectName, subjectColor: orig?.subjectColor ?? '#94a3b8',
        roomId: e.roomId ?? null, roomName: e.roomId ? roomById.get(e.roomId)?.name : undefined,
        teacherId: e.teacherId ?? null, teacherName: orig?.teacherName ?? lookup.userName(e.teacherId ?? undefined),
        isDoublePeriod: e.isDoublePeriod,
      }
    })
  }, [classById, roomById, lookup])

  const runRefine = async () => {
    if (!autoDraft?.cohortIds?.length) return
    try {
      const draftEntries: RefineDraftEntry[] = autoDraft.result.draftEntries.map(d => ({
        classId: d.classId, dayOfWeek: d.dayOfWeek, periodIdx: d.periodIdx, classSubjectId: d.classSubjectId,
        roomId: d.roomId, teacherId: d.teacherId, subjectName: d.subjectName, isDoublePeriod: d.isDoublePeriod,
      }))
      const res = await refineSvc.refine({ termId: autoDraft.termId, cohortIds: autoDraft.cohortIds, draftEntries })
      const decorated = decorateRefined(res.draftEntries, autoDraft.result.draftEntries)
      setAutoDraft(d => d ? { ...d, result: { ...d.result, draftEntries: decorated } } : d)
      setRefineStats(res)
      toast.success(`Refined — penalty score ${Math.round(res.scoreBefore)} → ${Math.round(res.scoreAfter)} (${res.improvementPct.toFixed(1)}% better)`)
    ...
    <div>
      ...
      {autoDraft ? (
        <AutoGenerateReview draft={autoDraft} onCommit={commitDraft} onDiscard={discardDraft} commitBusy={autoGen.busy === 'commit'} commitErr={commitErr}
          onRefine={autoDraft.cohortIds?.length ? runRefine : undefined} refineBusy={refineSvc.busy} refineStats={refineStats}
          diagBusy={diagJob.busy === 'run'} diagJobStage={diagJob.job?.currentStage} />
      ) : inSession && template ? (
        <>
          {/* Phase T8 — when this session's draft is itself a what-if patch, the affected-region reassurance
              stays visible for as long as it's the active draft, not just in the one-time review modal. */}
          <WhatIfVersionBanner version={activeVersion} cohortsByClassId={cohortsByClassId} />
          {/* Phase T7 §4 — the active edit session's own status bar (Approve/Publish/Discard/Undo/Locks/
              History) replaces the plain save/discard/publish row while a draft version is being hand-edited. */}
          <VersionStatusBar version={activeVersion} versionBusy={overrideSession.busy}
            onApprove={() => overrideSession.approve(activeVersion.id)} onPublish={() => overrideSession.publish(activeVersion.id)} onDiscard={() => overrideSession.discard(activeVersion.id)}
            onUndo={handleUndo} undoBusy={undoHook.busy === 'undo'} onShowHistory={() => setShowHistory(true)} onShowLocks={() => setShowLocks(true)}
            activeLockCount={locksHook.items.length} />

          <TimetableGrid template={template} days={sessionDays} dense renderCell={(d, p) => {
            const key = cellKey(d, p.idx)
            const entry = sessionByKey.get(key)
            if (!entry) return <FreeCell dense />
            const row = subjectRows.find(r => r.cs.id === entry.classSubjectId)
            const teacher = entry.teacherId ? lookup.userName(entry.teacherId) : row?.teacher?.name
            const room = entry.roomId ? roomList.find(r => r.id === entry.roomId)?.name : undefined
            const lock = findBlockingLock(locksHook.items, entry)
            return (
              <button onClick={() => setSessionCell({ entry, period: p })} className="group w-full text-left" aria-label={`Actions for ${DAY_LABELS[d]} ${p.label}`}>
                <PeriodCard title={row?.name ?? 'Unknown subject'} color={row?.color ?? '#94a3b8'} teacher={teacher} room={room}
                  time={`${p.start} – ${p.end}`} focusable={false}
                  className={`min-h-[76px] ${lock ? 'border-rose-300 dark:border-rose-500/40' : ''}`}
                  badge={lock ? <LockBadge reason={lock.reason} /> : undefined} />
              </button>
            )
          }} />
        </>
        ...
        <>
          {editLocked && overrideSession.governingPublished && (
            <PublishedImmutableBanner onStartSession={() => overrideSession.fork()} busy={overrideSession.busy === 'fork'} />
          )}
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2">
            ...
            <Link to="/portal/academic/timetable-preferences" className={ghostBtn} title="Weights that drive Mode 4 (Optimized) teacher scoring and soft-constraint refinement"><Sliders size={13} /> Preferences</Link>
            <Link to="/portal/academic/elective-blocks" className={ghostBtn} title="Grade-wide slots where students choose between parallel elective offerings"><Layers size={13} /> Elective Blocks</Link>
            <button onClick={() => overrideSession.fork()} disabled={!!busy || overrideSession.busy === 'fork' || (serverEntries.length === 0 && !editLocked)} className={ghostBtn}
              title="Start a tracked edit session — move/swap/lock periods with live clash detection, full undo, and an audit trail">
              <GitBranch size={13} /> {overrideSession.busy === 'fork' ? 'Starting…' : 'Edit Session'}
            </button>
            <button onClick={() => setWhatIfSetupOpen(true)} disabled={!!busy || !patchTargetVersion || whatIf.busy} className={ghostBtn}
              title={patchTargetVersion ? 'Model a change (teacher/room unavailable, event, requirement change) and get back the minimal patch — everything else stays untouched' : 'Publish a version (or start an edit session) first — what-if patches an existing version'}>
              <Zap size={13} /> What-If
            </button>
            <button onClick={openAutoSetup} disabled={!!busy || editLocked} className={ghostBtn} title="Generate a first-draft timetable to review before saving"><Wand2 size={13} /> Auto-Generate</button>
            <button onClick={openCopy} disabled={!!busy || editLocked || classList.length < 2 && termList.length < 2} className={ghostBtn} title="Copy another class's grid into this one (this grid must be empty)"><Copy size={13} /> Copy from…</button>
            <button onClick={togglePublish} disabled={!!busy || !data || dirty || editLocked || (serverEntries.length === 0 && !data.published)} className={ghostBtn} title={dirty ? 'Save first' : undefined}>
            ...
            <button onClick={save} disabled={!dirty || !!busy || editLocked} className="btn-ink flex items-center gap-2 px-5 py-2.5 text-[13.5px] font-semibold disabled:opacity-40"><Save size={14} /> {busy === 'save' ? 'Saving…' : 'Save'}</button>
          </div>
              ...
              <div className="grid gap-5 2xl:grid-cols-[minmax(0,1fr)_240px]">
                <div className="min-w-0">
                  <TimetableGrid template={template} days={days} dense renderCell={(d, p) => {
                    ...
                    if (!cell) {
                      return (
                        <button onClick={() => openCell(d, p)} disabled={editLocked} className="group relative w-full text-left disabled:cursor-not-allowed disabled:opacity-50" aria-label={`Add period ${DAY_LABELS[d]} ${p.label}`}>
                          <FreeCell dense />
                    ...
                    return (
                      ...
                      <button onClick={() => openCell(d, p)} disabled={editLocked} className="group w-full text-left disabled:cursor-not-allowed" aria-label={`Edit ${DAY_LABELS[d]} ${p.label}`}>
                        <PeriodCard title={row?.name ?? 'Unknown subject'} color={row?.color ?? '#94a3b8'} teacher={teacher} room={cell.roomId ? roomList.find(r => r.id === cell.roomId)?.name : undefined}
                          time={`${p.start} – ${p.end}`} focusable={false}
                          className={`min-h-[76px] ${editLocked ? 'opacity-60' : ''} ${clash ? 'border-rose-400 ring-2 ring-rose-200 dark:ring-rose-500/30' : changed ? 'border-indigo-300 dark:border-indigo-500/40' : ''}`}
      ...
      {/* Phase T7 — session cell actions (move/swap/regenerate/lock), locks manager, edit history */}
      {activeVersion && sessionCell && (
        <CellActionsModal open={!!sessionCell} onClose={() => setSessionCell(null)} entry={sessionCell.entry}
          classLabel={cls?.label ?? classId} subjectName={subjectRows.find(r => r.cs.id === sessionCell.entry.classSubjectId)?.name ?? 'Subject'}
          teacherName={sessionCell.entry.teacherId ? lookup.userName(sessionCell.entry.teacherId) : subjectRows.find(r => r.cs.id === sessionCell.entry.classSubjectId)?.teacher?.name}
          roomName={sessionCell.entry.roomId ? roomList.find(r => r.id === sessionCell.entry.roomId)?.name : undefined}
          period={sessionCell.period} periods={periods} days={ALL_WEEKDAYS} sessionEntries={sessionEntries} roomList={roomList} lookup={lookup}
          locks={locksHook.items} createLock={locksHook.createLock} releaseLock={locksHook.releaseLock} lockBusy={locksHook.busy}
          move={overrideActions.move} swap={overrideActions.swap} regenerateSlot={overrideActions.regenerateSlot} actionsBusy={overrideActions.busy}
          onSaved={reloadSession} />
      )}
      {activeVersion && (
        <LocksPanel open={showLocks} onClose={() => setShowLocks(false)} locks={locksHook.items} releaseLock={locksHook.releaseLock} lockBusy={locksHook.busy}
          teacherOptions={teacherOptions} roomOptions={roomList} createLock={locksHook.createLock} days={ALL_WEEKDAYS} />
      )}
      {activeVersion && (
        <HistoryPanel open={showHistory} onClose={() => setShowHistory(false)} events={editEvents.items} onRevertTo={handleRevertTo} revertBusy={undoHook.busy === 'revert'} />
      )}

      {/* Phase T8 — what-if trigger + result review, available whenever there's a version to patch
          (published or drafted), independent of whether an edit session is currently open. */}
      <WhatIfSetupModal open={whatIfSetupOpen} onClose={() => setWhatIfSetupOpen(false)} onRun={runWhatIf} busy={whatIf.busy}
        periods={periods.filter(p => p.kind === 'class')} days={ALL_WEEKDAYS} roomList={roomList} teacherOptions={teacherOptions}
        cohortOptions={yearCohorts.map(c => ({ id: c.id, name: c.name }))} targetVersion={patchTargetVersion} classId={classId} />
      <WhatIfReviewModal open={!!whatIfResult} onClose={() => setWhatIfResult(null)} result={whatIfResult} lookup={lookup} roomList={roomList} periods={periods} onOpenInGrid={openWhatIfInGrid} />
      ...
      <Modal open={autoSetupOpen} onClose={() => setAutoSetupOpen(false)} title="Auto-Generate Timetable">
        <div className="space-y-4">
          <Field label={`Cohorts · ${selectedCohortIds.length} selected`}>
            {yearCohorts.length === 0 ? (
              <p className="text-[12.5px] text-black/50 dark:text-white/50">No cohorts yet — every class has its own 1:1 section cohort automatically; define more under Classes &amp; Sections if you need cross-section scheduling.</p>
            ) : (
              <div className="thin-scroll max-h-48 space-y-1 overflow-y-auto rounded-2xl border border-black/[.08] dark:border-white/[.12] p-2">
                {yearCohorts.map(c => (
                  <label key={c.id} className="flex cursor-pointer items-center gap-2.5 rounded-xl px-2.5 py-2 hover:bg-black/[.03] dark:hover:bg-white/[.05]">
                    <input type="checkbox" checked={selectedCohortIds.includes(c.id)} onChange={() => toggleCohort(c.id)} className="h-4 w-4 accent-indigo-600" />
                    <span className="min-w-0 flex-1 truncate text-[13.5px] font-medium">{c.name}</span>
                    {c.autoGenerated && <Pill tone="slate">Auto</Pill>}
                  </label>
                ))}
              </div>
            )}
            <p className="mt-1.5 text-[12px] text-black/45 dark:text-white/45">Uses each cohort's Teaching Requirements (Classes &amp; Sections → Cohorts → Teaching Requirements) rather than the class's plain subject list — select this class's own cohort for a single-class run, or every cohort for a whole-year run. If a cohort has no Teaching Requirements yet, seed them from its existing subjects first (Teaching Requirements page → "Seed from existing subjects").</p>
          </Field>
          <Field label="Mode">
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={runAutoGenerate} disabled={autoGen.busy === 'generate' || (autoMode === 'full-regenerate' && !autoConfirm) || selectedCohortIds.length === 0}
 ...
 */
function AutoGenerateReview({ draft, onCommit, onDiscard, commitBusy, commitErr, onRefine, refineBusy, refineStats, diagBusy, diagJobStage }: {
  draft: { result: AutoGenerateResult; mode: AutoGenerateMode; termId: string; cohortIds?: string[]; diagnostics?: Diagnostic[] }
  onCommit: () => void
  onDiscard: () => void
  commitBusy: boolean
  commitErr: string | null
  /** Refinement needs `cohortIds` (phase-t5-full-solver.md §2) — every draft carries them since T10 §2,
   * but the prop stays optional defensively. */
  onRefine?: () => void
  refineBusy: boolean
  refineStats: RefineDraftResult | null
  /** Phase T6 §3/§4 — true while the diagnosis job (fired automatically when anything was left unplaced) is
   * running; `diagJobStage` is its last-known `currentStage` for the progress strip below. */
  diagBusy: boolean
  diagJobStage?: string
}) {
    ...
    <div className="space-y-5">
      <Card className="p-5">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <span className="flex-1" />
          {onRefine && (
            <button onClick={onRefine} disabled={refineBusy || commitBusy || result.draftEntries.length === 0} className={ghostBtn}
              title="Runs soft-constraint refinement (simulated annealing) over this draft — workload balance, lab-split avoidance, gap/preference penalties">
              <Sparkles size={13} /> {refineBusy ? 'Refining…' : refineStats ? 'Refine again' : 'Refine'}
            </button>
          )}
        ...
        {/* Phase T5 §2 — refinement stats folded into this SAME summary strip, not a bolted-on second
            panel (D9): total penalty score before/after, zero-hard-violation confirmation, and a
            per-grade-group breakdown since refinement runs decomposed by cohort-group. */}
        {refineStats && (
          <div className="mt-4 border-t border-black/[.06] dark:border-white/[.08] pt-4">
            <div className="flex flex-wrap items-center gap-2 text-[13px]">
              <Pill tone={refineStats.improvementPct > 0 ? 'green' : 'slate'}>
                Penalty score {Math.round(refineStats.scoreBefore)} → {Math.round(refineStats.scoreAfter)} ({refineStats.improvementPct.toFixed(1)}% better)
              </Pill>
              <Pill tone={refineStats.hardViolationsAfter === 0 ? 'green' : 'rose'}>
                {refineStats.hardViolationsAfter === 0 ? 'Zero hard-constraint violations' : `${refineStats.hardViolationsAfter} hard violation${refineStats.hardViolationsAfter === 1 ? '' : 's'}`}
              </Pill>
            </div>
            <div className="mt-2.5 flex flex-wrap gap-x-4 gap-y-1 text-[12px] text-black/50 dark:text-white/50">
              {refineStats.groups.map(g => (
                <span key={g.groupKey}>
                  <span className="font-semibold text-black/70 dark:text-white/70">{g.gradeLabel}</span>: {Math.round(g.scoreBefore.total)} → {Math.round(g.scoreAfter.total)} ({g.improvementPct.toFixed(1)}%, {g.iterations.toLocaleString()} iterations)
                </span>
              ))}
            </div>
          </div>
      ...
      {/* Phase T6 §3/§4 — while the diagnosis job runs, a real (if currently brief, since generation is
          still synchronous — see jobs.ts) progress strip rather than a silent wait before the diagnostic
          panel appears. */}
      {diagBusy && (
        <Card className="p-4">
          <div className="flex items-center gap-3 text-[13px] font-medium text-black/60 dark:text-white/60">
            <Sparkles size={14} className="shrink-0 animate-pulse text-indigo-500" />
            <span className="flex-1">{diagJobStage ?? 'Diagnosing why some periods couldn\'t be placed…'}</span>
          </div>
          <div className="mt-2.5"><Progress pct={60} /></div>
        </Card>
      )}
